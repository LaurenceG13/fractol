...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/10/28 09:12:34 by lagirard          #+#    #+#             */
/*   Updated: 2017/10/28 19:34:46 by lagirard         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/fractol.h"

int		ft_nb_points_juju(t_param *s)
{
	int		x;
	int		y;
	int		point;
	...
	while (x < s->image_x)
	{
		...
		while (y < s->image_y)
		{
			if (ft_compute_juju(s, x, y) != s->iter_max)
			{
				ft_init_xy(s, x, y);
				point = point + 1;
			}
...
int		ft_coord_juju_bis(t_param *s, int point, int x, int y)
{
	int		i;
	int		p;

	i = ft_compute_juju(s, x, y);
	p = point;
	if (i != s->iter_max)
	{
		s->tmp_pixels[p][0] = x;
...
int		ft_coord_juju(t_param *s)
{
	...
	x = 0;
	y = 0;
	s->size_coord = ft_nb_points_juju(s);
	ft_init_juju(s, x, y);
	...
	while (x < s->image_x)
	{
		...
		while (y < s->image_y)
		{
			point = ft_coord_juju_bis(s, point, x, y);
			y++;
		}
		x++;
	}
...
int		ft_fractal_juju(t_param *s)
{
	...
	mlx_string_put(s->mlx, s->win, 5, 5, 0X00FFFFFF, "JUJU");
