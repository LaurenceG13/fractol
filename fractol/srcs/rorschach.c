...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/10/31 23:20:53 by lagirard          #+#    #+#             */
/*   Updated: 2017/10/31 23:20:55 by lagirard         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/fractol.h"

int		ft_nb_points_ror(t_param *s)
{
	int		x;
	int		y;
	int		point;
	...
	while (x < s->image_x)
	{
		...
		while (y < s->image_y)
		{
			if (ft_compute_ror(s, x, y) != s->iter_max)
			{
				ft_init_xy(s, x, y);
				point = point + 1;
			}
...
int		ft_coord_ror_bis(t_param *s, int point, int x, int y)
{
	int		i;
	int		p;

	i = ft_compute_ror(s, x, y);
	p = point;
	if (i != s->iter_max)
	{
		s->tmp_pixels[p][0] = x;
...
int		ft_coord_ror(t_param *s)
{
	...
	x = 0;
	y = 0;
	s->size_coord = ft_nb_points_ror(s);
	ft_init_ror(s, x, y);
	...
	while (x < s->image_x)
	{
		...
		while (y < s->image_y)
		{
			point = ft_coord_ror_bis(s, point, x, y);
			y++;
		}
		x++;
	}
...
int		ft_fractal_ror(t_param *s)
{
	...
	mlx_string_put(s->mlx, s->win, 5, 5, 0X00FFFFFF, "RORSCHACH");
