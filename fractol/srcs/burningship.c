...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/10/27 17:57:48 by lagirard          #+#    #+#             */
/*   Updated: 2017/10/31 21:54:50 by lagirard         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/fractol.h"

int		ft_nb_points_burning(t_param *s)
{
	int		x;
	int		y;
	int		point;
	...
	while (x < s->image_x)
	{
		...
		while (y < s->image_y)
		{
			if (ft_compute_burning(s, x, y) != s->iter_max)
			{
				ft_init_xy(s, x, y);
				point = point + 1;
			}
...
int		ft_coord_burning_bis(t_param *s, int point, int x, int y)
{
	int		i;
	int		p;

	i = ft_compute_burning(s, x, y);
	p = point;
	if (i != s->iter_max)
	{
		s->tmp_pixels[p][0] = x;
...
int		ft_coord_burning(t_param *s)
{
	int		x;
	int		y;
	int		point;
	...
	x = 0;
	y = 0;
	s->size_coord = ft_nb_points_burning(s);
	ft_init_burning(s, x, y);
	...
	while (x < s->image_x)
	{
		...
		while (y < s->image_y)
		{
			point = ft_coord_burning_bis(s, point, x, y);
			y++;
		}
		x++;
	}
...
int		ft_fractal_burning(t_param *s)
{
	...
	mlx_string_put(s->mlx, s->win, 5, 5, 0X00FFFFFF, "BURNINGSHIP");
	mlx_string_put(s->mlx, s->win, 155, 5, 0X00FFFFFF, "imax: ");
	str = ft_itoa(s->iter_max);
	mlx_string_put(s->mlx, s->win, 210, 5, 0X00FFFFFF, str);
	free(str);
	return (0);
}
