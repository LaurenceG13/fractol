...
int		ft_nb_points_tricorn(t_param *s)
{
	int		x;
	int		y;
	int		point;
	...
	while (x < s->image_x)
	{
		...
		while (y < s->image_y)
		{
			if (ft_compute_tricorn(s, x, y) != s->iter_max)
			{
				ft_init_xy(s, x, y);
				point = point + 1;
			}
...
int		ft_coord_tricorn_bis(t_param *s, int point, int x, int y)
{
	int		i;
	int		p;

	i = ft_compute_tricorn(s, x, y);
	p = point;
	if (i != s->iter_max)
	{
		s->tmp_pixels[p][0] = x;
...
int		ft_coord_tricorn(t_param *s)
{
	int		x;
	int		y;
	int		point;
	...
	x = 0;
	y = 0;
	s->size_coord = ft_nb_points_tricorn(s);
	ft_init_tricorn(s, x, y);
	...
	while (x < s->image_x)
	{
		...
		while (y < s->image_y)
		{
			point = ft_coord_tricorn_bis(s, point, x, y);
			y++;
		}
		x++;
	}
...
int		ft_fractal_tricorn(t_param *s)
{
	...
	mlx_string_put(s->mlx, s->win, 5, 5, 0X00FFFFFF, "TRICORN");
